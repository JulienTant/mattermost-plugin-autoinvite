...
func (p *Plugin) ConfigurationWillBeSaved(newCfg *model.Config) (*model.Config, error) {
	cfg, ok := newCfg.PluginSettings.Plugins[root.Manifest.Id]
	if !ok {
		return nil, nil
	}

	teamNameItf, ok := cfg["teamname"]
	if !ok {
		return nil, nil
	}

	teamname, ok := teamNameItf.(string)
	if !ok {
		return nil, nil
	}

	if teamname == p.configuration.TeamName {
		return nil, nil
	}

	if teamname == "" {
		p.API.LogDebug("teamname is empty, no check to make")
		return nil, nil
	}
